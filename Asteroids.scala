...
case class Bullet(sprite: ImageSprite, dir: Double) {
  ...
  def moveShip = {
    ...
    val x = 0.04*math.sin(dir)
    val y = -0.04*math.cos(dir)
    // Move the Bullet
  ...
  // Set the location of a bulletss
  def setLoction(x: Double, y: Double) = sprite.setLocation(x,y)

  // Does this Bullet (its sprite) intersect the specified Sprite
  def intersects(s: Sprite) = sprite.intersects(s)
}

case class BulletBoss(sprite: ImageSprite, dir: Double) {
  // Method in Bullet class to translate the bullet
  def moveBoss = {
    // (x,y) coordinates of the Bullet
    val x = 0.04*math.sin(dir)
...
class Asteroids extends Game(500,500) {
  val ship = new ImageSprite("galaga_ship.png");
  var numEnemies = 4
  var enemy: Array[ImageSprite] = Array.tabulate(numEnemies)(x => new ImageSprite("galaga_dragonfly.png"))
  var numAs = 3
  ...
  def moveShip {
    if(keyPressed){
      ...
      c match {
        ...
        case 'g' => addGame(new AsteroidsMiniBoss)
        finishGame()
        case _ =>
      }
    }
